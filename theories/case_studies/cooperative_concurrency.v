...
Section typing.
  ...
  Definition iter_ty τ := (∀R: θ, (τ -{ ¡ θ }-> 𝟙) → List τ -{ ¡ θ }-∘ 𝟙)%T.
  
  Definition next_ty := (𝟙 → 𝟙)%T.

  Definition add_ty := ((𝟙 ⊸ 𝟙) → 𝟙)%T.

  Definition resume_task_ty := (∀T: α, '! α → ('! α ⊸ 𝟙) → 𝟙)%T.
  
  Definition runner_ty := (∀T: α, (𝟙 -{ coop }-∘ '! α) → '! α)%T.
  ...
  Lemma async_typed :
    ⊢ ⊨ᵥ async : (∀T: α, (𝟙 -{ coop }-∘ '! α) -{ coop }-> Promise ('! α)).
  Proof.
  ...
  Proof.
    ...
    - iApply sem_typed_sub_u2aarr.
      ...
      rewrite -/(C 𝟙). iApply sem_typed_TApp; first solve_copy.
  ...
  Proof.
    ...
    - assert (Hrw : [resume_task;add;next;main] = [resume_task;add;next] ++ [main]) by done.
      ...
      iApply (sem_typed_ufun_poly_rec (λ β, Promise ('! β)) (λ _, ⊥) (λ β, (𝟙 -{ coop }-∘ '! β) ⊸ 𝟙)); 
        solve_sidecond.
      { iIntros. iApply copy_ty_uarr. }
     simpl. iIntros (β) "/=". 
     set promise := ("promise", Promise ('! β)).
     set fulfill := ("fulfill", ∀T: β', Promise ('! β') → (𝟙 -{ coop }-∘ '! β') ⊸ 𝟙).
  ...
  - set fulfill := ("fulfill", ∀T: α, Promise ('! α) → (𝟙 -{ coop }-∘ '! α) ⊸ 𝟙).
